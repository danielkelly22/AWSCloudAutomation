...
# Subnet Shares
#-----------------------------------
resource "aws_ram_resource_share" "prod_omni_subnets_share" {
  allow_external_principals = false
  name                      = "ProdOmniSubnetShare"
  tags = {
    Name = "ProdOmniSubnetShare"
  }
}
resource "aws_ram_principal_association" "prod_omnius_association" {
  principal          = "675938983696"
  resource_share_arn = aws_ram_resource_share.prod_omni_subnets_share.id
}
resource "aws_ram_resource_association" "prod_omni_subnet_a_association" {
  resource_arn       = aws_subnet.OmniSubnetA.arn
  resource_share_arn = aws_ram_resource_share.prod_omni_subnets_share.id
}
resource "aws_ram_resource_association" "prod_omni_subnet_b_association" {
